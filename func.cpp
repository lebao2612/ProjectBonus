...
void readCourseAccount(string a, SVList& svlist) {
	...
	cout <<left<<setw(20)<< "Ma mon hoc" << setw(30) << "Ten mon hoc" << setw(15) << "Lop hoc" << setw(15) << "Giao vien" << setw(10) << "So tin" << setw(10) << "Toi da" << setw(10) << "Nam hoc" << setw(15) << "Thoi gian hoc" << endl;
...
void saveSVVaoFile(string a, SVList sv) {
	...
	f << "STT,MSSV,Ten,Ho,Gioi tinh,Ngay sinh,ID" << endl;
	SinhVien* temp = sv.Head;
	while (temp != NULL) {
...
void xoa1SVKhoaHoc(SVList& svlist) {
	...
	int dem = 1;
	f << "STT,MSSV,Ten,Ho,Gioi tinh,Ngay sinh,ID,DiemGK,DiemCK,DiemCong,GPA" << endl;
	while (i != NULL) {
...
void xuatHocSinhMonHoc(SVList& svlist) {
	string a;
	cout << "Nhap ma mon hoc: ";
	getline(cin, a);
	...
	string No, Maso, Ten, Ho, GioiTinh, Ngaysinh, ID, GK, CK, Cong, GPA;
	while (!f.eof()) {
		getline(f, No, ',');
		if (No == "")
			break;
		...
		getline(f, GPA, '\n');
		string hoten = Ho + " " + Ten;
		cout << left << setw(10) << No << setw(15) << Maso << setw(30) << hoten << setw(15) << GioiTinh << setw(20) << Ngaysinh << setw(20) << ID << setw(10) << GK << setw(10) << CK << setw(10) << Cong << setw(10) << GPA << endl;
	}
	f.close();
	cout << endl;
	cout << "1. Cap nhat diem" << endl;
	cout << "2. Thoat" << endl;
	int chon;
	cout << "Nhap lua chon: ";
	cin >> chon;
	if (chon == 1) {
		cin.ignore();
		capnhatDiemKhoaHoc(a, svlist);
	}
}


void xemDiemSinhVien(string a, SVList& svlist) {
	fstream f1;
	string ma, tenMH, lop, gv, sotin, hsmax, nam, thoigian;
	SinhVien* temp = svlist.Head;
	while (temp != NULL) {
		if (temp->MSSV == a)
			break;
	...
	Course* i = temp->data.Head;
	cout << left << setw(20) << "Ma MH" <<setw(30)<<"Ten MH"<< setw(10) << "Diem GK" << setw(10) << "Diem CK" << setw(10) << "Diem Cong" << setw(10) << "GPA" << endl;
	while (i != NULL) {
		f1.open("CoursesList.csv", ios::in);
		if (f1.fail()) {
			cout << "Khong mo duoc file courseslist" << endl;
		...
		while (!f.eof()) {
			...
			if (maso == a) {
				...
				getline(ss, cong, ',');
				getline(ss, gpa, '\n');
				cout << left << setw(20) << i->maMH << setw(30) << tenMH << setw(10) << gk << setw(10) << ck << setw(10) << cong << setw(10) << gpa << endl;
				break;
			}
		}
		i = i->pnext;
...
//a la ma khoa hoc
void capnhatDiemKhoaHoc(string a, SVList& sv) {
	SVList svkhoahoc;
	khoiTaoSVList(svkhoahoc);
	luuSinhVienKhoaHoc(a, svkhoahoc);
	luuDiemSinhVienKhoaHoc(a, sv);
	string b = "Courses\\" + a + ".csv";
	...
	f << "STT,MSSV,Ten,Ho,Gioi tinh,Ngay sinh,ID,DiemGK,DiemCK,DiemCong,GPA" << endl;
	SinhVien* temp1 = svkhoahoc.Head;
	while (temp1 != NULL) {
		temp = sv.Head;
		while (temp != NULL) {
			if (temp->MSSV == temp1->MSSV) {
				f << temp->No << "," << temp->MSSV << "," << temp->Ten << "," << temp->Ho << "," << temp->Gioitinh << "," << temp->Ngaysinh << "," << temp->ID << ",";
				Course* i = temp->data.Head;
				while (i != NULL) {
					if (i->maMH == a)
						f << i->diem.GK << "," << i->diem.CK << "," << i->diem.Cong << "," << i->diem.GPA << endl;
					i = i->pnext;
				}
			}
			temp = temp->pnext;
...
// a la ma mon hoc
void luuSinhVienKhoaHoc(string a, SVList& sv) {
	fstream f;
	string b = "Courses\\" + a + ".csv";
	f.open(b, ios::in);
	if (f.fail()) {
...
void luuDiemSinhVienKhoaHoc(string a, SVList& svlist) {
	SinhVien* temp = svlist.Head;
	while (temp != NULL) {
		Course* temp1 = temp->data.Head;
		while (temp1 != NULL) {
			...
			while (!f.eof()) {
				...
				getline(f, line);
				stringstream ss(line);
				string no, mssv, ten, ho, gioitinh, ngaysinh, id;
				getline(ss, no, ',');
				if (no == "")
					break;
				temp->No = stoi(no);
				...
				if (mssv == temp->MSSV) {
					...
					getline(ss, ngaysinh, ',');
					getline(ss, id, ',');
					string diemGK, diemCK, diemCong, GPA;
					getline(ss, diemGK, ',');
					getline(ss, diemCK, ',');
					getline(ss, diemCong, ',');
					getline(ss, GPA, '\n');
					...
					if (diemGK == "")
						...
						temp1->diem.GK = stof(diemGK);
					if (diemCK == "")
						temp1->diem.CK = -1;
					else
						temp1->diem.CK = stof(diemCK);
					if (diemCong == "")
						temp1->diem.Cong = -1;
					else
						temp1->diem.Cong = stof(diemCong);
					if (GPA == "")
						temp1->diem.GPA = -1;
					else
						temp1->diem.GPA = stof(GPA);
					break;
				}
			}
			temp1 = temp1->pnext;
